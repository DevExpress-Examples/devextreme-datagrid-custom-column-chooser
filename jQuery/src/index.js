$(() => {
  $('#gridContainer').dxDataGrid({
    ...
    onToolbarPreparing(e) {
      const customColumnChooser = createColumnChooser(e.component);
...
function createColumnChooser(grid) {
  const columns = grid.option('columns').map((column) => ({
    dataField: column,
  ...
  // Create Popup
  const popup = $('#customColumnChooser').dxPopup({
  ...
  // Sync selected items
  grid.on('optionChanged', (e) => {
    if (e.name === 'columns' && e.fullName.includes('visible')) {
      const columnIndex = parseInt(e.fullName.charAt(8), 10);
      if (e.value) {
